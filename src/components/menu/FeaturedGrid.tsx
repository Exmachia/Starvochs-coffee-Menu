...
function EvenCardContent({ item }: { item: MenuItem }) {
  return (
    <>
      <div className="flex aspect-[1/0.85] items-center justify-center rounded-[15px] bg-[radial-gradient(circle_at_50%_32%,var(--color-surface-2),var(--color-chip-bg))]">
        {item.glass && <GlassIcon {...item.glass} className="h-[62%] w-[62%]" />}
      </div>
      <div className="text-balance font-display text-[15px] font-semibold text-heading min-[520px]:text-[16.5px]">
        {item.name}
      </div>
      <p className="line-clamp-2 text-[12.5px] leading-snug text-ink-muted min-[520px]:text-[13px]">
        {item.desc}
      </p>
      <div className="mt-auto flex items-center justify-between gap-2 pt-1">
        <span className="text-[14px] font-bold tabular-nums text-accent min-[520px]:text-[15px]">
          {item.priceSD ? `S $${item.priceSD.s} / D $${item.priceSD.d}` : priceLabel(item)}
        </span>
        <span className="flex flex-none text-ink-muted">
          <IconChevron className="block h-3.5 w-3.5" />
...
export function FeaturedGrid({
  ...
  /** "grid" is the original equal-column card grid, used by the menu page's own
   * Más Vendidos tab. "even" gives every item the same size/shape with a richer
   * card (icon, name, price, and description) — used on the home page. */
  layout?: 'grid' | 'even'
}) {
  if (layout === 'even') {
    return (
      <div className="grid grid-cols-2 gap-3 min-[680px]:grid-cols-4 min-[680px]:gap-3.5">
        {items.map((item) =>
          action.type === 'link' ? (
            <Link key={item.id ?? item.name} href={action.href} className={evenCardClass}>
              <EvenCardContent item={item} />
            </Link>
          ) : (
            <button
