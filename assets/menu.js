...
const ICO = {
  ...
  share:'<svg viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="1.8" stroke-linecap="round" stroke-linejoin="round"><circle cx="18" cy="5" r="2.6"/><circle cx="6" cy="12" r="2.6"/><circle cx="18" cy="19" r="2.6"/><path d="M8.3 10.7l7.4-4.4M8.3 13.3l7.4 4.4"/></svg>',
...
const CATEGORIES = [
  ...
  },
  {
    ...
    items:[
      {id:'bf-lattes-rocas', name:'Lattes en las Rocas', priceSD:{s:'50',d:'55'}, note:'Vainilla · Moka · Chocolate · Caramelo · Menta · Chocomenta · Crema Irlandesa · Bombón Tostado · Crema de Coco',
      ...
      {id:'bf-chocolate-artesanal', name:'Chocolate Artesanal en las Rocas', price:'50',
        desc:'Nuestro chocolate artesanal, servido bien frío sobre hielo.', lleva:['Chocolate artesanal','Leche','Hielo']},
      {id:'bf-smoothies', name:'Smoothies', price:'50', note:'Frutos Rojos · Mango · Maracuyá · Kiwi',
        desc:'Fruta natural licuada con hielo: fresca, ligera y llena de sabor.', lleva:['Fruta natural','Hielo']},
      {id:'bf-sodas-italianas', name:'Sodas Italianas', price:'50', note:'Frutos Rojos · Maracuyá · Mango · Kiwi',
  ...
  },
  {
    ...
    items:[
      ...
      {id:'fr-con-sabor', name:'Con Sabor', price:'50', note:'Vainilla · Moka · Chocolate · Bombón Tostado · Chocomenta · Caramelo · Menta · Crema Irlandesa',
        desc:'Nuestro frappé clásico con el sabor que más se te antoje.', lleva:['Espresso','Leche','Hielo','Sabor a elegir']},
      {id:'fr-especiales', name:'Especiales', price:'55', note:'Cookies & Cream · Frappé Chai · Taro · Matcha',
...
/* Flat id -> {item, cat} index, built once, so a shared link's #hash can find
   and open the right product regardless of which category it lives in. */
const ITEM_INDEX = {};
...
/* Same idea as priceLabel(), but price12_16 needs its size labels (12oz/16oz,
   S/D...) spelled out for share text, since there's no column header to lean on. */
function priceShareText(item, cat){
  if(item.price12_16) return `${cat.sizeLabels[0]} $${item.price12_16.a} / ${cat.sizeLabels[1]} $${item.price12_16.b}`;
...
CATEGORIES.forEach(cat=>{
  ...
  head.innerHTML = cat.icon + `<h2>${cat.label}</h2>` + '<div class="section-head-rule"></div>';
...
let toastTimer=null;
let currentItem=null, currentCat=null;

function showToast(msg){
  toast.textContent = msg;
  toast.hidden = false;
  // Force a reflow so the "show" transition replays even if a toast is already visible.
...
async function shareItem(item, cat){
  ...
  if(navigator.share){
    try{ await navigator.share({title:item.name, text, url}); }
...
modalShare.addEventListener('click', ()=>{ if(currentItem) shareItem(currentItem, currentCat); });

function openModal(item, cat){
  lastFocused = document.activeElement;
  currentItem = item; currentCat = cat;
...
function openExtraModal(key){
  lastFocused = document.activeElement;
  currentItem = null; currentCat = null;
  const ex = EXTRAS[key];
...
/* A shared link (#coco-matcha) lands here, filters to that product's category
   so it's visible in context, and opens its modal directly. */
function openFromHash(){
  const id = decodeURIComponent(location.hash.slice(1));
  if(!id || !ITEM_INDEX[id]) return;
  const {item, cat} = ITEM_INDEX[id];
  selectTab(cat.id);
  openModal(item, cat);
}
openFromHash();
